...
<div class="relative overflow-hidden bg-gradient-to-r from-teal-500 to-cyan-600 rounded-3xl p-6 md:p-8 text-white shadow-xl shadow-teal-500/10">
    <div class="relative z-10 space-y-2 md:max-w-xl">
        ...
        <p class="text-teal-50/90 text-sm md:text-base leading-relaxed">
            Welcome to your personalized HealthMesh health portal. View your clinical logs, schedule hospital checkups, and manage medical bills in one secure hub.
        </p>
    </div>
    
    <!-- Premium Decorative SVG Icon -->
    <div class="absolute right-0 bottom-0 top-0 opacity-15 md:opacity-20 trangray-x-10 trangray-y-4 md:trangray-x-0 flex items-center justify-center pointer-events-none">
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
    ...
    <!-- Appointments Card -->
    <div class="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm hover:shadow-md hover:-trangray-y-1 transition-all duration-300 flex items-center gap-4">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-3xl border border-gray-200 p-6 lg:col-span-2 shadow-sm space-y-4">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('patient.appointments') }}" class="text-xs font-semibold text-teal-600 hover:text-teal-700 transition-colors">View All &rarr;</a>
        </div>
                ...
                <div class="py-4 first:pt-0 last:pb-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-0.5">{{ $apt->doctor->specialization->name }} &bull; {{ $apt->patientEnrollment->hospital->name }}</p>
                    ...
                    <div class="flex items-center gap-2.5 self-end sm:self-center">
                        <span class="px-2.5 py-1 text-xs font-semibold rounded-full uppercase tracking-wider {{ $apt->status === 'confirmed' ? 'bg-green-50 text-green-700' : 'bg-blue-50 text-blue-700' }}">
                            {{ $apt->status }}
                        </span>
                        ...
                        @if ($apt->status === 'scheduled')
                            <form action="{{ route('patient.appointments.cancel', $apt->id) }}" method="POST" class="m-0" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan jadwal konsultasi ini?')">
                                @csrf
                                <button type="submit" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all cursor-pointer" title="Cancel Appointment">
    ...
    <div class="bg-white rounded-3xl border border-gray-200 p-6 shadow-sm space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3">
            <!-- Book Appointment -->
            <a href="{{ route('patient.appointments') }}" class="group flex items-center gap-3 p-3.5 rounded-2xl bg-teal-50/50 hover:bg-teal-500 border border-teal-100/50 transition-all duration-300">
                <span class="p-2.5 bg-teal-500 text-white group-hover:bg-white group-hover:text-teal-600 rounded-xl transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </span>
                <div>
                    <h4 class="font-bold text-gray-800 text-sm group-hover:text-teal-900 transition-colors">Schedule Checkup</h4>
                    <p class="text-[11px] text-gray-400 group-hover:text-teal-800/80 mt-0.5">Book consultations & check schedules</p>
                </div>
            </a>

            <!-- Medical History -->
            <a href="{{ route('patient.medical-records') }}" class="group flex items-center gap-3 p-3.5 rounded-2xl bg-blue-50/50 hover:bg-blue-500 border border-blue-100/50 transition-all duration-300">
                <span class="p-2.5 bg-blue-500 text-white group-hover:bg-white group-hover:text-blue-600 rounded-xl transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                </span>
                <div>
                    <h4 class="font-bold text-gray-800 text-sm group-hover:text-blue-900 transition-colors">Medical Records</h4>
                    <p class="text-[11px] text-gray-400 group-hover:text-blue-800/80 mt-0.5">Review diagnostic & treatment logs</p>
                </div>
            </a>

            <!-- Manage Bills -->
            <a href="{{ route('patient.bills') }}" class="group flex items-center gap-3 p-3.5 rounded-2xl bg-amber-50/50 hover:bg-amber-500 border border-amber-100/50 transition-all duration-300">
                <span class="p-2.5 bg-amber-500 text-white group-hover:bg-white group-hover:text-amber-600 rounded-xl transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                </span>
                <div>
                    <h4 class="font-bold text-gray-800 text-sm group-hover:text-amber-900 transition-colors">Billing & Payments</h4>
                    <p class="text-[11px] text-gray-400 group-hover:text-amber-800/80 mt-0.5">Settle bills securely with Midtrans</p>
                </div>
            </a>

            <!-- Check Prescriptions -->
            <a href="{{ route('patient.prescriptions') }}" class="group flex items-center gap-3 p-3.5 rounded-2xl bg-emerald-50/50 hover:bg-emerald-500 border border-emerald-100/50 transition-all duration-300">
                <span class="p-2.5 bg-emerald-500 text-white group-hover:bg-white group-hover:text-emerald-600 rounded-xl transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"></path></svg>
                </span>
                <div>
                    <h4 class="font-bold text-gray-800 text-sm group-hover:text-emerald-900 transition-colors">Active Prescriptions</h4>
                    <p class="text-[11px] text-gray-400 group-hover:text-emerald-800/80 mt-0.5">Check dosages, drugs & rules</p>
                </div>
            </a>
        </div>
    </div>
