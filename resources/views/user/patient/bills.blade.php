...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <div class="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm flex items-center gap-4 hover:shadow-md transition-all duration-300">
            ...
            <div>
                <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">Unpaid Bills</p>
                <h3 class="text-2xl font-black text-gray-800 mt-0.5">{{ $unpaidCount }}</h3>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm flex items-center gap-4 hover:shadow-md transition-all duration-300">
            ...
            <div>
                <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">Total Outstanding</p>
                <h3 class="text-xl font-black text-red-600 mt-0.5">Rp {{ number_format($totalUnpaidAmount, 0, ',', '.') }}</h3>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm flex items-center gap-4 hover:shadow-md transition-all duration-300">
            ...
            <div>
                <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">Paid Bills</p>
                <h3 class="text-2xl font-black text-gray-800 mt-0.5">{{ $paidCount }}</h3>
            </div>
        </div>
    ...
    <div class="space-y-4">
        ...
        @forelse ($unpaidBills as $bill)
            <div class="bg-white border border-gray-200 rounded-3xl shadow-sm overflow-hidden hover:shadow-md transition-all duration-300">
                ...
                <div class="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-gray-100 bg-gray-50/20">
                    <div class="space-y-1">
                        ...
                        <p class="text-xs text-gray-400">Statement Date: {{ $bill->created_at->format('d M Y') }} &bull; Hospital: {{ $bill->patientEnrollment->hospital->name }}</p>
                    </div>

                    <div class="flex flex-col sm:flex-row sm:items-center md:justify-end gap-4">
                        <div class="space-y-0.5 sm:text-right">
                            <span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Amount Due</span>
                ...
                <div class="px-6 py-4 space-y-3">
                    <h5 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-1">Statements Breakdown</h5>
                        ...
                        @foreach ($bill->billItems as $item)
                            <div class="px-4 py-3 flex items-center justify-between text-xs font-medium">
                                <div class="space-y-0.5 max-w-sm">
                                    <span class="text-gray-800 font-semibold">{{ $item->description }}</span>
                                ...
                                <div class="text-right">
                                    <span class="block text-gray-800 font-bold">Rp {{ number_format($item->subtotal, 0, ',', '.') }}</span>
                                    <span class="block text-[9px] text-gray-400 font-semibold">{{ $item->quantity }} x Rp {{ number_format($item->unit_price, 0, ',', '.') }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>

                    <div class="flex items-center justify-between text-xs font-semibold text-gray-500 px-1 pt-2">
        ...
        @empty
            <div class="bg-white rounded-3xl border border-gray-200 p-8 text-center space-y-3 shadow-sm">
                <div class="inline-flex p-3 bg-green-50 text-green-600 rounded-full">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                </div>
                <p class="text-sm font-semibold text-gray-500">All bills are fully settled! No outstanding amounts.</p>
            </div>
        @endforelse
    </div>
    ...
    <div class="space-y-4 pt-2">
                        ...
                        <tbody class="divide-y divide-gray-100 text-xs font-semibold text-gray-700">
                            @foreach ($paidBills as $paid)
                                <tr class="hover:bg-gray-50/30 transition-colors">
                                    <td class="px-6 py-4 text-gray-800 font-bold">INV-{{ sprintf('%06d', $paid->id) }}</td>
                                    ...
                                    <td class="px-6 py-4 uppercase">
                                        <span class="px-2 py-0.5 bg-gray-100 border border-gray-200/40 text-[10px] rounded text-gray-500 font-bold uppercase">{{ str_replace('_', ' ', $paid->payment_method ?? 'Cash') }}</span>
                                    </td>
                                    <td class="px-6 py-4 font-mono text-[10px] text-gray-400 truncate max-w-[120px]" title="{{ $paid->reference_number }}">{{ $paid->reference_number ?? '-' }}</td>
                                    <td class="px-6 py-4 text-right text-green-600 font-bold">Rp {{ number_format($paid->total_amount, 0, ',', '.') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        @else
            <div class="bg-white border border-gray-200 rounded-3xl p-8 text-center space-y-2 shadow-sm">
