...
<head>
    ...
    <style>
        ...
        ::-webkit-scrollbar-thumb { background: rgba(20,184,166,.25); border-radius: 10px; }
...
<div class="flex min-h-screen">
    ...
    <aside id="patient-sidebar" class="w-[17.5rem] bg-[#111827] text-white flex flex-col fixed h-full z-20 shadow-2xl transition-transform duration-300 -trangray-x-full md:trangray-x-0">
        ...
        <div class="mx-4 mt-4 mb-2 p-3 rounded-xl bg-white/5 border border-white/10 flex items-center gap-3">
            ...
            <div class="min-w-0">
                ...
                <p class="text-[11px] text-gray-400 truncate">Patient Account</p>
            </div>
        </div>
        ...
        <nav class="flex-1 px-3 space-y-0.5 overflow-y-auto">
            @php
                $navItems = [
                    ...
                    ['route' => 'patient.medical-records', 'icon' => 'fa-file-medical',                 'label' => 'Medical Records'],
                    ['route' => 'patient.bills',           'icon' => 'fa-file-invoice-dollar',          'label' => 'Billing & Bills'],
                    ['route' => 'patient.prescriptions',   'icon' => 'fa-prescription-bottle-medical',  'label' => 'Prescriptions'],
    ...
    <div class="flex-1 md:ml-[17.5rem] flex flex-col min-h-screen">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="relative" id="profile-dropdown-wrapper">
                    ...
                    <div id="profile-dropdown-menu"
                         class="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-xl shadow-xl py-1.5 hidden z-50">
                        <div class="px-4 py-2 border-b border-gray-100">
                            <p class="text-xs text-gray-400">Logged in as</p>
                            <p class="text-xs font-semibold text-gray-800 truncate">{{ $patientUser?->email }}</p>
                        </div>
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="m-0">
                            @csrf
                            <button type="submit"
                                    class="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 text-left cursor-pointer">
        ...
        <main class="flex-1 p-6 bg-gray-50">
            ...
            {{-- Flash Alerts --}}
            @if (session('success'))
                <div class="flex items-center gap-3 mb-5 p-4 bg-teal-50 border border-teal-200 rounded-xl text-teal-800 shadow-sm">
                    <i class="fa-solid fa-circle-check text-teal-500"></i>
                    <span class="text-sm font-medium">{{ session('success') }}</span>
                </div>
            @endif
            @if (session('error'))
                <div class="flex items-center gap-3 mb-5 p-4 bg-red-50 border border-red-200 rounded-xl text-red-800 shadow-sm">
